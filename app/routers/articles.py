...
async def create_article(article_create: schemas.ArticleCreate, session: AsyncSession = Depends(get_session)):
    ...
    if existing_article:
        logger.info(f"tried to create an already published article {article_create.title}")
    ...
    if not user:
        logger.info(f"tried to create an article of a non existing author {article_create.author_id}")
